...
def add_technical_features_v2(
    ...
    """
    Version 2: Clean features WITHOUT price-level leakage.

    Key changes from v1:
    - Removes: log_close, all absolute MAs/EMAs, absolute Bollinger, absolute Fibonacci levels
    - Keeps: returns, normalized ratios, relative distances, technical indicators
    - Adds: Fibonacci composite, momentum, return acceleration

    Returns DataFrame with ~15-18 features that don't leak price level information.
    """
    ...
    # ========== LOG RETURNS (keep these - they're historical) ==========
    with np.errstate(divide="ignore", invalid="ignore"):
    ...
    # Return acceleration (change in returns)
    df["log_ret_accel"] = df["log_ret_1"].diff(1)

    # ========== MOVING AVERAGE RATIO (normalized, not absolute) ==========
    sma_20 = df["close"].rolling(20, min_periods=20).mean()
    df["ma_ratio_20"] = (df["close"] - sma_20) / sma_20.replace(0, np.nan)

    # ========== BOLLINGER BANDS (only width and std, not absolute levels) ==========
    bb_m = df["close"].rolling(20, min_periods=20).mean()
    bb_std = df["close"].rolling(20, min_periods=20).std()
    bb_up = bb_m + 2.0 * bb_std
    bb_dn = bb_m - 2.0 * bb_std
    df["bb_width"] = (bb_up - bb_dn) / bb_m.replace(0, np.nan)
    df["bb_std_pct"] = bb_std / df["close"].replace(0, np.nan)  # normalized std

    # ========== RSI (scale-independent) ==========
    try:
    ...
    try:
        atr = atr_simple(df["high"].values, df["low"].values, df["close"].values, length=14)
        df["atr_pct"] = atr / df["close"].replace(0, np.nan)
    except Exception:
        raw_atr = (df["high"] - df["low"]).rolling(14, min_periods=14).mean()
        df["atr_pct"] = raw_atr / df["close"].replace(0, np.nan)
    ...
    # ========== FIBONACCI COMPOSITE ==========
    # Calculate Fibonacci levels for distance computation only (not as features)
    highs = df["high"].rolling(fib_lookback, min_periods=fib_lookback).max()
    lows = df["low"].rolling(fib_lookback, min_periods=fib_lookback).min()
    range_ = (highs - lows).replace(0, np.nan)
    ...
    # Stack all distances
    if distances:
        dist_arr = np.vstack([d.values for d in distances]).T  # shape (N, 8)

        # Composite: weighted sum with inverse distance weighting
        # Levels closer to current price get higher weight
        with np.errstate(invalid="ignore", divide="ignore"):
            weights = 1.0 / (1.0 + np.abs(dist_arr))  # closer = higher weight
            fib_composite = np.nansum(weights * np.sign(dist_arr), axis=1) / np.nansum(weights, axis=1)
        df["fib_composite"] = fib_composite
    else:
        df["fib_composite"] = np.nan
    ...
    # ========== SELECT FINAL FEATURES (drop close, high, low) ==========
    feature_cols = [
        "log_ret_1", "log_ret_5", "momentum_10", "log_ret_accel",
        "ma_ratio_20", "bb_width", "bb_std_pct",
        "rsi_14", "atr_pct", "raw_vol_10", "raw_vol_30",
        "fib_composite", "td_buy_setup", "td_sell_setup"
    ]

    # Keep only feature columns that exist
    existing_features = [c for c in feature_cols if c in df.columns]
    result = df[existing_features].copy()
    ...
    if out_dtype == "float32":
        for c in result.select_dtypes(include=["float64"]).columns:
...
def directional_accuracy_loss(pred_ret: "torch.Tensor", target_ret: "torch.Tensor", alpha: float = 0.3) -> "torch.Tensor":
    """
    Penalizes predictions with incorrect directional sign.

    Args:
        pred_ret: Predicted log returns (N,)
        target_ret: Target log returns (N,)
        alpha: Weight for this loss component
    ...
    target_sign = torch.sign(target_ret)

    # Fraction of incorrect directions
    incorrect = (pred_sign != target_sign).float()
    return alpha * incorrect.mean()


def variance_matching_loss(pred_ret: "torch.Tensor", target_ret: "torch.Tensor", beta: float = 0.2) -> "torch.Tensor":
    """
    Penalizes predictions with variance significantly different from target.
    Encourages model to make bolder predictions (not just hedge with small values).

    Args:
        pred_ret: Predicted log returns (N,)
        target_ret: Target log returns (N,)
        beta: Weight for this loss component
    ...
    target_std = target_ret.std()

    # Penalize difference in standard deviations
    return beta * torch.abs(pred_std - target_std)
...
def combined_loss(
    ...
    1. MSE for return prediction
    2. MSE for volatility prediction
    3. Directional accuracy penalty
    4. Variance matching penalty

    Args:
        pred_ret: Predicted returns (N,)
        pred_vol: Predicted volatility (N,)
        target_ret: Target returns (N,)
        target_vol: Target volatility (N,)
        alpha_vol: Weight for volatility loss
        alpha_dir: Weight for directional loss
        alpha_var: Weight for variance matching loss
    ...
    # Directional loss
    loss_dir = directional_accuracy_loss(pred_ret, target_ret, alpha=alpha_dir)

    # Variance matching loss
    loss_var = variance_matching_loss(pred_ret, target_ret, beta=alpha_var)

    # Combine all components
    total_loss = loss_ret + alpha_vol * loss_vol + loss_dir + loss_var
